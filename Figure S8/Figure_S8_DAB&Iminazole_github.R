data<-read.csv("Green&Etiolated leaves_Imidazole_phenotype.csv")

data$Type<-factor(data$Type,levels=c("Green","Etio"))
data$treat2<-factor(data$treat2,levels=c("treat","ck"))
data$group1<-factor(data$group1,levels=c("Green_DAB","Green_Imi","Etio_DAB","Etio_Imi"))

data$time <- as.numeric(data$time)
pd <- position_dodge(0.7)

pdf("Rplot02_Green & Etiolated-Imi.pdf",width=10,height=3.5)
