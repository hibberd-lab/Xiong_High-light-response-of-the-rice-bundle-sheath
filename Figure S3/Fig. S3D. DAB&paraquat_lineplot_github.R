data<-read.csv("DAB&paraquat_phenotype.csv")

data$treat2<-factor(data$treat2,levels=c("HL","CK"))
data$treat1<-factor(data$treat1,levels=c("DAB","paraquat"))

data$time <- as.numeric(data$time)
pd <- position_dodge(0.7)

pdf("Rplot02_Green leaves_DAB&paraquat.pdf",width=10,height=3.5)
